...
def _retrieve_iot_certificate(hass, id_token):
    """Retrieve the certificate to connect to IoT."""
    # TODO error handling
    return _make_api_call(hass, id_token, 'device/create').json()


def _make_api_call(hass, id_token, path, method='POST'):
    """Make a request to our API server."""
    mode = util.get_mode(hass)
